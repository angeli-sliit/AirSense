                                    ...
                                    {!cmpCombined && (
                                        <div id="comparison-individual-charts" className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                            {Object.entries(cmpChartData).map(([cityName, data]) => (
                                                <DataChart key={cityName} data={data} title={`${cityName} PM2.5 Levels`} cityName={cityName} />
                                            ))}
                                        </div>
                                    )}
                                ...
                                <div className="flex justify-end mt-4">
                                    <AuroraButton
                                        onClick={() => {
                                            ...
                                            // Prepare chart data for the report page
                                            const chartData = {
                                                individual: fcRes?.byCity ? Object.fromEntries(
                                                    Object.entries(fcRes.byCity).map(([cityName, series]) => [
                                                        cityName, 
                                                        Array.isArray(series) ? series : []
                                                    ])
                                                ) : {},
                                                combined: fcCombined && Array.isArray(forecastChartData) && forecastChartData.length > 0 ? forecastChartData : null
                                            };

                                            // Navigate to the print forecast report page with forecast data
