...
def forecast(payload: ForecastIn, plan: Plan = Depends(get_plan), db: Session = Depends(get_db)):
...
def forecast_multi(payload: ForecastMultiIn, plan: Plan = Depends(get_plan), db: Session = Depends(get_db)):
